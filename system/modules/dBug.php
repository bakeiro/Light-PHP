...
class dBug {
	...
	function __construct($var,$forceType="",$bCollapsed=false) {
		...
		$arrAccept=array("array","object","xml","error"); //array of variable types that can be "forced"
	...
	//if variable is an array type
	function varIsError($var) {
		$var_ser = serialize($var);
		array_push($this->arrHistory, $var_ser);
		
		$this->makeTableHeader("error","Error");
		if(is_array($var)) {
			foreach($var as $key=>$value) {
				$this->makeTDHeader("error",$key);
	...
	function initJSandCSS() {
		echo <<<SCRIPTS
			...
			<style type="text/css">
				...
				/* Error */
				table.dBug_error { background-color:#006600; }
				table.dBug_error td { background-color:#FFFFFF; }
				table.dBug_error td.dBug_errorHeader { background-color:#d9534f; color: #FFFFFF }
				table.dBug_error td.dBug_errorKey { background-color:#CCFFCC; }
